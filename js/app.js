...
function SetMarkers(position) {
    ...
    this.infowindowcontent = '<h4 class="infowindowhead">' + this.markName + '</h4>' +
    '<img src=' + self.imgURL + '>';
    ...
    this.marker.addListener('click', function() {
        self.infowindowcontent = '<h4 class="infowindowhead">' + self.markName + '</h4>' +
        '<img src=' + self.imgURL + '>';
	...
	this.visibility = ko.computed(function() {
		...
		if (!filter) {
			...
			return ko.utils.arrayFilter(self.locations(), function(item) {
				var string = item.markName.toLowerCase();
